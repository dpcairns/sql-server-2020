...
app.get('/banjos', async(req, res) => {
  try {
    ...
    select banjos.id, banjos.noise_level, brands.name as brand
      from banjos
      join brands
      on brands.id = banjos.brand_id
...
// we need all the brands so we can make a brands dropdown on the front end
...
app.post('/banjos/', async(req, res) => {
  try {
    ...
    const newBrandId = req.body.brand_id;
    ...
    [newBrandId, newNoiseLevel, newOwnerId]);
...
app.put('/banjos/:id', async(req, res) => {
  try {
    ...
    `, 
    // use the weird $ syntax and this array to prevent SQL injection (i.e. Bobby "DROP TABLES")
    [newBrand, newNoiseLevel, newOwnerId, req.params.id]);
